...
Public Class Thumbn
    ...
    Shared pathPhoto As String = "\\labo4\photos-rh\"
    Shared pathThumbs As String = "\\labo4\photos-rh\Thumbnails\"
    ...
    Shared Sub ComparePhoto(ByVal matricule As String, ByVal objDirEnt As DirectoryEntry)
        Dim ctrlZonesLabo As Boolean = Directory.Exists("\\labo4\photos-rh\")
        If ctrlZonesLabo = False Then
            pathPhoto = "\\nxprod\photos-rh\"
            pathThumbs = "\\nxprod\photos-rh\Thumbnails\"
        End If
        Try
    ...
    Shared Function LireImage(ByVal imageFile As String) As Byte()
        Try
            ...
            Return imageF
        Catch
            Commun.Journal("ERREUR : Erreur de lecture du fichier image : " & imageFile, True)
        End Try
        imageFile = Nothing
    End Function
End Class
