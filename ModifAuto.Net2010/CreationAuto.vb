...
Public Class CreationAuto
    ...
    Public Shared objGroupe As Object
    Public Shared tabDatabase(1, 8) As String
    ...
    Shared Sub Start()
        ...
        Catch ex As Exception
            Commun.Journal("ERREUR : FTP : Recuperation du fichier : " & ex.Message, True)
        End Try
        ...
        For Each utilisateurProv As SearchResult In result
            ...
            If dateExpD < DateAdd(DateInterval.Month, -3, Now) And usr1.AccountDisabled = True And dateExpT = "COMPTE PROVISOIRE" Then
        ...
        Application.Exit()
    End Sub
    Public Shared Sub fichierRep(ByVal type As String)
        ...
        Next
    End Sub
    Public Shared Sub createCompte(ByVal fichier As String)
            ...
            While Not EOF(4)
                ...
                If InStr(Tc(i), ",") <> 0 Then 'evite les lignes blanches
                    ...
                    Try
                        ...
                        Dim tempCtrlPathExist As Boolean = ""
                        '           UserExist controle l'existance d'un utilisateur uniquement dans l'OU Utilisateurs
                        tempCtrlPathExist = DirectoryEntry.Exists(Champc(1) & " " & Champc(0) & " (Provisoire)")

                        objOUUtilisateurs = New DirectoryEntry("LDAP://OU=Users Provisoires,OU=Utilisateurs,DC=igbmc,DC=u-strasbg,DC=fr")


                        If tempCtrlPathExist = False Then
                            ...
                            objUser.CommitChanges()
                        Else
                            Dim CNCompteProv = "CN =" & Champc(1) & " " & Champc(0) & " (Provisoire),OU=Users Provisoires,OU=Utilisateurs,DC=igbmc,DC=u-strasbg,DC=fr"
                            objUser = New DirectoryEntry("LDAP://" & CNCompteProv)
                            objUser.Rename("LDAP://CN=" & Champc(1) & " " & Champc(0))
                                objUser.CommitChanges()
                                objUser.MoveTo(objOUUtilisateurs)
                                objUser.CommitChanges()
                            Commun.Journal("ERREUR : Creation de compte : le compte existait déja, il a été modifié : " & Champc(2), True)
                        End If
                    ...
                    Catch ex As Exception
                        Commun.Journal("ERREUR : Creation de compte : le compte existait déja, une erreur s'est produite : " & Champc(2) & " : " & ex.Message, True)
                    End Try
                    ...
                    Try
                        ...
                        objUser.CommitChanges()
                    Catch
                        Commun.Journal("ERREUR : Creation de compte : Attributs : " & Champc(2), True)
                    End Try
                    ...
                    Try
                        ...
                        objUser.CommitChanges()
                    Catch
                        Commun.Journal("ERREUR : Ajout PosixAccount au compte : Attributs : " & Champc(2), True)
                    End Try
                    ...
                    Try
                        ...
                        objUser.CommitChanges()
                    Catch
                        Commun.Journal("ERREUR : Creation de l'utilisateur Unix : " & Champc(2), True)
                    End Try
                    ...
                    If EqDescr <> " " Then
                        Try
                            ...
                            objGroupe = Nothing
                        Catch
                            Commun.Journal("ERREUR : l'utilisateur était deja membre de l'equipe comptable : " & Champc(2), True)
                        End Try
                    End If
                    ...
                    Catch e As Exception
                        Commun.Journal("ERREUR : ajout à l'equipe ADMINISTRATIVE de l'utilisateur : " & e.Message & " : " & Champc(2), True)
                    End Try

                    'Determiner le 
                    Try
                        Dim objUserNT = GetObject("WinNT://igbmc/" & Champc(2))
                        'objUserNT.Put("userFlags", &H10040) 'Flag sur "l'utilisateur ne peut pas changer de mot de passe" et "le mot de passe n'expire jamais"
                        objUserNT.Put("userFlags", &H10000) 'Flag sur "le mot de passe n'expire jamais"
                        objUserNT.setinfo()
                        objUserNT = Nothing
                    Catch
                    End Try
                    ...
                    Try
                        ...
                        Ldap = Nothing
                    Catch e As Exception
                        Commun.Journal("ERREUR : Creation de compte : Creation des alias mail : " & e.Message & " : " & Champc(2), True)
                    End Try
                    ...
                    Catch e As Exception
                        Commun.Journal("ERREUR : Creation de compte : Creation du compte mail : " & e.Message & " : " & Champc(2), True)
                    End Try
                    ...
                    Try
                        ...
                        objGroupe = Nothing
                    Catch e As Exception
                        Commun.Journal("ERREUR : Creation de compte : Ajout liste de diffusions ANNOUCE : " & e.Message & " : " & Champc(2), True)
                    End Try
                    ...
                    Try
                        ...
                        objGroupe = Nothing
                    Catch e As Exception
                        Commun.Journal("ERREUR : Creation de compte : Ajout liste de diffusions CE : " & e.Message & " : " & Champc(2), True)
                    End Try
                    ...
                    Try
                        ...
                        End If
                    Catch e As Exception
                        Commun.Journal("ERREUR : Creation de compte : Ajout liste de diffusions PHD ou POSTDOC : " & e.Message & " : " & Champc(2), True)
                    End Try
    ...
    Public Shared Sub SupprimCompte(ByVal fichier As String)
        ...
        If FileLen(fichier) > 0 Then
            ...
            While Not EOF(3)
                ...
                If Len(Ts(k)) > 2 Then 'evite les lignes blanches
                    ...
                    Catch ex As Exception
                        Commun.Journal("ERREUR : Suppression de compte : " & Ts(k) & " : " & ex.Message, True)
                    End Try
                    ...
                    Catch ex As Exception
                        Commun.Journal("ERREUR : Suppression du compte ADM : " & Ts(k) & "Adm : " & ex.Message, True)
                    End Try
                    ...
                    Try
                        ...
                        End If
                    Catch ex As Exception
                        Commun.Journal("ERREUR : Suppression du dossier Profil Itinerant : " & Ts(k) & " : " & ex.Message, True)
                    End Try
                    ...
                    Try
                        ...
                        End If
                    Catch ex As Exception
                        Commun.Journal("ERREUR : Suppression du dossier Profil Itinerant Adm : " & Ts(k) & "Adm : " & ex.Message, True)
                    End Try
                End If
            End While
            dirSearcher = Nothing
    ...
    Public Shared Sub CtrlGroupAdmins()
        ...
        For compteur = 0 To 4
            ...
            If InStr(listMembresAutorises, "Administrateur,steph,stephadm") <> 0 Then
                ...
                Dim monGroupe As DirectoryEntry = New DirectoryEntry("LDAP://" & Commun.TransformeSAMACCOUNTenCN(samaccountGroup))
                Try
    ...
    Public Shared Function UserExist1(ByVal CNAVerifier As String) As String
    ...
    End Function
    Public Shared Sub NbUserDatabaseExchange()
    ...
    End Sub
    Public Shared Function TrouverPetiteDatabase() As String
    ...
    End Function
    Public Shared Sub DelUnixUser(ByVal login As String)
        Try
            ...
            LdapU = Nothing
        Catch ex As Exception
            Commun.Journal("ERREUR : Suppression du compte Unix : " & ex.Message & " : " & login, True)
        End Try

    End Sub
    Public Shared Sub commandePWSMailbox(ByVal aliasMail As String, ByVal db As String)
        ...
        Try
            ...
            pRunspace.Close()
        Catch e As Exception
            Commun.Journal("ERREUR : Creation de compte : Creation du compte mail : " & e.Message & " : " & aliasMail, True)
        End Try

    End Sub
    Public Shared Function PresenceExceptionUser(ByVal login As String) As Boolean
